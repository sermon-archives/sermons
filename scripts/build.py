...
def outline_html(text):
    ...
    for line in text.splitlines():
        ...
        if re.match(r"^#{1,3}\s",line):
            stripped=re.sub(r"^#{1,3}\s*","",line); lines.append(f"<h3>{html.escape(stripped)}</h3>")
        elif re.match(r"^\d+[.)]\s+",line): lines.append(f"<p><strong>{html.escape(line)}</strong></p>")
        elif line.startswith(('-', '*')): lines.append(f"<p>· {html.escape(line[1:].strip())}</p>")
        else: lines.append(f"<p>{html.escape(line)}</p>")
    return ''.join(lines)
